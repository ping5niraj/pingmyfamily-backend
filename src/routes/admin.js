...
// GET /api/admin/pending-relations  ← NEW
router.get('/pending-relations', adminAuth, async (req, res) => {
  const { data, error } = await supabase
    .from('pmf_relationships')
    .select(`
      id, relation_type, relation_tamil, created_at,
      is_offline, offline_name,
...
router.get('/kuthams', adminAuth, async (req, res) => {
  const { data: kuthams } = await supabase.from('pmf_kuthams').select('id, name, created_at').order('name', { ascending: true });
  ...
  const result = (kuthams || []).map(k => ({ ...k, user_count: countMap[k.name] || 0 }));
...
router.get('/users', adminAuth, async (req, res) => {
  ...
  let query = supabase.from('pmf_users').select('id, name, phone, gender, kutham, district, created_at, status').order('created_at', { ascending: false }).limit(100);
